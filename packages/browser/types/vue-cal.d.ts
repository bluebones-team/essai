...
declare module 'vue-cal' {
  import { type DefineSetupFnComponent, type VNode } from 'vue';

  // const VueCal = Vue.extend<VueCalProps>(Vue);
  export namespace vuecal {
    type FormattedDate = string; // eg (e.g. 2020-09-18)
    type View = 'years' | 'year' | 'month' | 'week' | 'day';
    type WeekDays = 1 | 2 | 3 | 4 | 5 | 6 | 7;

    interface SplitDaysAttributes {
      id: string | number;
      class: string;
      label: string;
      hide: boolean;
    ...
    interface Event {
      _eid?: string;
      start: Date; // '2018-11-19 12:00', // Required.
      end: Date; // '2018-11-19 14:00', // Required.
      ...
      contentFull?: string; // opens in a dialog
    ...
    type OnEventClick = (event: Event, e: MouseEvent) => void;

    interface EventReadyChanged {
      id: View;
    ...
    interface EventTitleChange {
      event: Event;
      oldTitle: string;
    }

    interface EventDurationChange extends EventChange {
      oldDate: Date;
    }
    ...
    interface VueCalEvent extends Event {
      daysCount: number;
      deleting: boolean;
      draggable: boolean;
      dragging: boolean;
    ...
    interface Slots extends Record<string, any> {
      title?(e: { title: string; view: EventReadyChanged }): VNode;
      'time-cell'?(e: { hours: number; minutes: number }): VNode;
      'cell-content'?(e: {
        cell: any;
        view: EventReadyChanged;
        events: Event[];
        goNarrower: () => void;
      }): VNode;
    }
    ...
    interface Instance {
       ...
       * @param {Boolean} next
       */
      previousNext(next: boolean = true);
       ...
       */
      switchToNarrowerView(date: string | Date = null);
       ...
       */
      loadLocale(locale: string | object);

      //endregion
    }
  }
  declare const VueCal: DefineSetupFnComponent<
    vuecal.Props,
    vuecal.EventListeners,
    vuecal.Slots
  >;
